...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:variable name="viewUrl">
    ...
    <xsl:choose>
      <xsl:when test="/data/calendar/event/parentId &gt; 0"><xsl:value-of select="/data/calendar/event/parentId" /></xsl:when>
      <xsl:otherwise><xsl:value-of select="/data/calendar/event/id" /></xsl:otherwise>
    </xsl:choose>
	</xsl:variable>
  <xsl:variable name="detailViewUrl">
		<xsl:value-of select="concat($detailBaseUrl, '?eventID=', $detailId)" />
  </xsl:variable>
