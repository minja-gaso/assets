...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="primary_content_navigation">
    ...
    <ul class="nav nav-tabs">
      ...
      -->
      <xsl:choose>
        <xsl:when test="$SCREEN = 'TOPICS'">
          <li role="presentation" class="active"><a href="#">Topics</a></li>
        </xsl:when>
        <xsl:otherwise>
          <li role="presentation"><a href="javascript:switchTab('TOPICS');submitForm();">Topics</a></li>
        </xsl:otherwise>
      </xsl:choose>
        <!--
  ...
  <xsl:template name="list_nav">
    <ul class="nav nav-tabs">
      <li role="presentation" class="active"><a href="#">My Blogs</a></li>
